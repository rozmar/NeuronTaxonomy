...
function Leaves = backwardSearchBestCombinationNotStrictFromClusters(X, features, Clusters, iter, silent)
  ...
  while 1
    ...
    for i = 1 : size(Q,1)
      ...
      for j = 1 : size(possibleEdges,1)
        ...
        for i = 1 : iter
          M(i,j) = moreDimensionIsBetterFromClusters(X,f1,f2,'mul',Clusters,features);
        end
        %evaluate the indicator matrix: if m(j) is 1, in all iteration was the new combination is better
        m = mean(M(:,j));
%         
%         for i2 = 1 : length(Clusters(length(f2)).Index)
%           f = features(Clusters(length(f2)).Index(i2).features)';
%           if mean(sort(f) == sort(f2))==1
%             m = m * mean(Clusters(length(f2)).Index(i2).probs);
%             break;
%           end
%         end
