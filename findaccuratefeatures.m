...
for i = 1 : size(F,1)
    ...
    if sum(isnan(W_f))>size(W_f,1)*0.1
       continue 
    end
    IDX = [];
    P = [];
    ...
    cfm = calcConfusionMatrixClustering(W,idx,y);
    ...
    acc = (cfm(1)+cfm(4))/sum(sum(cfm));

    fprintf('%d was the missed elements, F1-score %.2f, accuracy: %.2f\n', sum(cfm(2:3)), ((2*precision*recall)/(precision+recall)), acc);

    %if F1>0.99
    if acc>0.9
        AccurateFeatures = [ AccurateFeatures ; testingCombination ];
    end
end